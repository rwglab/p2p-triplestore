...
public class CommutePutCommand extends PutCommand {

	public CommutePutCommand(ServerInfo caller, ServerInfo classLoaderHost, Entry entry) {
		super(caller, classLoaderHost, entry);
	}

	@Override
	protected void put(final DHash dhash) {
		dhash.getStorage().commutePut(entry, getCaller().getIdentifier(), new LockingStorage.Returner<Object>() {
			public void call(Object b) {
				CommutePutCommand.this.setReturnValue(null);
				CommutePutCommand.this.returnHome(dhash);
			}
		});
	}
